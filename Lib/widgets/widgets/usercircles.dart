...
class usercircle extends StatefulWidget {
  const usercircle({
    key, //String username, List<Map<String, dynamic>> userDataList
...
class _usercircleState extends State<usercircle> {
  ...
  Widget build(BuildContext context) {
    return Container(
      child: Stack(
        children: widget.userDataList.map((user) {
          return widget.usernames.reversed
                      .where((af) => user['username1'] == af["writer"])
                      .length >
                  5
              ? Container(
                  child: Row(
                    children: [
                      ...widget.usernames.reversed
                          .where((af) => user['username1'] == af["writer"])
                          .take(4)
                          .toSet()
                          .toList()
